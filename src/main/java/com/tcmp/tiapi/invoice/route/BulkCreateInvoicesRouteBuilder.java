...
public class BulkCreateInvoicesRouteBuilder extends RouteBuilder {
  ...
  public void configure() {
    ...
    from(uriFrom).routeId("bulkCreateInvoicesBulkInTI").threads(THREAD_POOL_SIZE_FOR_BULK_OPERATIONS)
      ...
      // Validate rows
      .process(exchange -> {
        InvoiceCreationRowCSV invoice = exchange.getIn().getBody(InvoiceCreationRowCSV.class);
        try (var validatorFactory = Validation.buildDefaultValidatorFactory()) {
          var validator = validatorFactory.getValidator();
          Set<ConstraintViolation<InvoiceCreationRowCSV>> violations = validator.validate(invoice);
          String violationMessage = violations.toString();

          if (!violations.isEmpty()) validationErrors.add(violationMessage);
        }
      })
      // Map to TI Message
