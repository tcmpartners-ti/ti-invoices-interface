  ...
  @Query(
      value =
          ...
          SELECT
            ...
            program.PROG_TYPE AS programmeType,
            programSubtype.DESCRIPT AS programmeSubtype,
          ...
          FROM
            INVMASTER invoice
          JOIN SCFCPARTY buyer ON buyer.KEY97 = invoice.BUYER
          JOIN SCFCPARTY seller ON seller.KEY97 = invoice.SELLER
          JOIN SCFPROGRAM program ON program.KEY97 = invoice.PROGRAMME
          JOIN SCFPROSTYP programSubtype ON program.SUBTYPECAT = programSubtype.CODE
          JOIN EXTMASTER extra ON invoice.KEY97 = extra.MASTER
          JOIN SCFMAP relationship ON relationship.PARTY = buyer.KEY97
            AND relationship.CPARTY = seller.KEY97
          JOIN EXTPROGRAMME extraPro ON program.ID = extraPro.PID
          JOIN MASTER master ON invoice.KEY97 = master.KEY97
            AND master.STATUS = 'LIV'
          LEFT JOIN INTERE59 intSche ON relationship.KEY97 = intSche.SCFMAP
            AND intSche.OBSOLETE = 'N'
          LEFT JOIN INT_TIER intTier ON intSche.KEY29 = intTier.owner
          LEFT JOIN FNCEMASTER finance ON finance.FINCEEVKEY = invoice.FINCE_EV
          WHERE
  ...
  @Query(
      value =
          ...
          SELECT
            ...
            program.PROG_TYPE AS programmeType,
            programSubtype.DESCRIPT AS programmeSubtype,
            extraPro.EXFINDAY AS programmeExtraFinancingDays,
            (finance.FINCE_AMT / 100) AS financeAmount,
